...
def dann(encoder, classifier, source_train_loader, target_train_loader, sum_pooling_mode,
    ...
    for epoch in range(params.epochs):
        print('Epoch : {}'.format(epoch))
        set_model_mode('train', [encoder, classifier, sumdiscriminator])
        ...
        for batch_idx, (source_data, target_data) in enumerate(zip(source_train_loader, target_train_loader)):
            ...
            if (batch_idx + 1) % 50 == 0:
                if sum_pooling_mode == 1 or sum_pooling_mode == 2:
                    print(
                        '[{}/{} ({:.0f}%)]\tLoss: {:.6f}\tClass Loss: {:.6f}\tSum Loss: {:.6f}'.format(
                    ...
                    print('[{}/{} ({:.0f}%)]\tLoss: {:.6f}\tClass Loss: {:.6f}'.format(
                        batch_idx * len(target_image), len(target_train_loader.dataset),
                        100. * batch_idx / len(target_train_loader), total_loss.item(), class_loss.item()))
        ...
        if (epoch + 1) % 10 == 0:
            ...
            result_list, current_acc = only_sumpooling_test.tester(encoder, classifier, sumdiscriminator,
                                                                   source_test_loader, target_test_loader, epoch,
                                                                   sum_pooling_mode,
                                                                   training_mode='dann')
            if current_acc > best_acc:
                best_acc = current_acc
                save_model(epoch, encoder, classifier,sumdiscriminator ,'dann', save_dir)
            # visualize(epoch, encoder, 'source', save_name)
